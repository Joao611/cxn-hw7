...
The time complexity is O(|V|.(|V|-1)/2) = O(|V|<sup>2</sup>) while the space complexity is also O(|V|.(|V|-1)/2) = O(|V|<sup>2</sup>) due to the worst case of having big enough desired degrees to allow every possible edge to exist.

The Fisher Yates shuffle wouldn't help improve it. A possible algorithm using it could be:
```{r, eval=FALSE}
...
```
It stores every possible edge in a list at the beginning, making the space complexity O(|V|.log(|V|)). The time complexity is O(|V|.log(|V|)).
